...
import { machineHasCamera, machineIsEnclosed } from "@/lib/machine-accessories";
...
function detectFeatureIds(machine: Machine): StandoutFeatureId[] {
  ...
  const camera = machineHasCamera(machine);
...
export function getMachineStandoutFeatures(
  ...
  if (machine.standoutFeatures?.length) {
    return machine.standoutFeatures;
  }
  ...
  return ids
    .map((id) => {
      ...
      return { id, title: copy.title, body: copy.body, priority: FEATURE_COPY[id].priority };
    })
