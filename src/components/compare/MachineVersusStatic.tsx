...
interface MachineVersusStaticProps {
  machines: Machine[];
  locale: Locale;
  dict: Dictionary;
}
...
/** Non-interactive, server-rendered comparison table for static duel pages. */
export function MachineVersusStatic({ machines, locale, dict }: MachineVersusStaticProps) {
            ...
            {sections.map((section) => (
              <Fragment key={section.id}>
                <tr className="bg-amber-50/80 dark:bg-amber-950/25">
                  <td
                    colSpan={colCount + 1}
                    className="px-4 py-3 text-xs font-bold uppercase tracking-widest text-amber-900 dark:text-amber-200"
                  >
                    {section.title}
                  </td>
                </tr>
